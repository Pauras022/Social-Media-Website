...
function Friends() {
  ...
  useEffect(() => {
    ...
    async function sentFriendRequest() {
      const res = await fetchSentFriendRequests();
      if (res.data) {
        userDispatch({
          type: "SET_FRIENDS_REQUEST_SENT",
    ...
    async function recommendedUser() {
      const res = await fetchRecommendedUsers();
      if (res && res.data) {
        userDispatch({
          type: "SET_USERS",
  ...
  const { acceptFriendRequest, declineFriendRequest, cancelFriendRequest } =
    useFriendAction();
  ...
  const metaData = (
    <div className={classes.sidebarContainer}>
      ...
      {userState.sentFriendRequests.length ? (
        <>
          <Typography variant="h6">Sent Friend Request</Typography>
          {userState.sentFriendRequests.map((request) => (
